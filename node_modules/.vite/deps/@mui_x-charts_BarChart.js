...
function useBarSeries() {
  ...
  return React8.useMemo(() => series.bar, [series.bar]);
}
...
var HighlightedContext = React13.createContext({
  isInitialized: false,
  data: {
...
function HighlightedProvider({
  ...
  const series = useSeries();
  const seriesById = React14.useMemo(() => {
  ...
  const providerValue = React14.useMemo(() => {
    return {
      isInitialized: true,
      data: {
        highlightScope,
...
function BarLabelPlot(props) {
  ...
  return (0, import_jsx_runtime11.jsx)(React26.Fragment, {
...
function BarPlot(props) {
  ...
  return (0, import_jsx_runtime12.jsxs)(React27.Fragment, {
...
function ChartsPiecewiseGradient(props) {
  ...
  return (0, import_jsx_runtime15.jsx)("linearGradient", {
    ...
    children: colorMap.thresholds.map((threshold2, index) => {
      ...
      return (0, import_jsx_runtime15.jsxs)(React31.Fragment, {
...
var useChartContainerHooks = (ref, plugins) => {
  const svgRef = React35.useRef(null);
...
var ChartContainer = React36.forwardRef(function ChartContainer2(props, ref) {
  const {
    width,
    height,
    series,
...
var useChartContainerDimensions = (inWidth, inHeight) => {
  ...
  const displayError = React37.useRef(false);
  const [width, setWidth] = React37.useState(0);
  const [height, setHeight] = React37.useState(0);
  const computeSize = React37.useCallback(() => {
...
var ResponsiveChartContainer = React38.forwardRef(function ResponsiveChartContainer2(props, ref) {
  const {
    width: inWidth,
    height: inHeight
...
function ChartsText(props) {
  ...
  const wordsByLines = React39.useMemo(() => getWordsByLines({
    style,
...
function useMouseTracker() {
  const svgRef = useSvgRef();
  const [mousePosition, setMousePosition] = React44.useState(null);
  React44.useEffect(() => {
    const element = svgRef.current;
    if (element === null) {
      return () => {
...
function useColorProcessor(seriesType) {
  const colorProcessors = React46.useContext(ColorContext);
  if (!seriesType) {
...
function DefaultChartsLegend(props) {
  ...
  const theme = useTheme();
  const isRTL = theme.direction === "rtl";
  const labelStyle = React51.useMemo(
  ...
  const padding = React51.useMemo(() => getStandardizedPadding(paddingProps), [paddingProps]);
  const getItemSpace = React51.useCallback((label, inStyle = {}) => {
  ...
  const [seriesWithPosition, legendWidth, legendHeight] = React51.useMemo(() => {
    let x = 0;
    let y = 0;
...
var BarChart = React60.forwardRef(function BarChart2(props, ref) {
  const {
    chartContainerProps,
    barPlotProps,
    axisClickHandlerProps,
