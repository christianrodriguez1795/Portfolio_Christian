...
const EditProject = ({ auth, project, skills }) => {
  const { data, setData, put, errors } = useForm({
    ...
    url: project.url,
    image: null,
    skill_id: project.skills.map(skill => skill.id), // Ajustado para habilidades múltiples
  });

  const [preview, setPreview] = useState(project.image ? `${project.image}` : null);
  ...
  const handleSubmit = (e) => {
    ...
    axios.post(route('projects.update', project.id), formData, {
      headers: {
  ...
  return (
    <AuthenticatedLayout user={auth.user} header={<h2 className="text-black dark:text-white font-semibold text-xl leading-tight">Editar proyecto</h2>}>
          ...
          <form onSubmit={handleSubmit} noValidate className="w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 mb-4 gap-4 md:gap-6">
              ...
              <CustomSelectMultiple
                ...
                value={data.skill_id}
                onChange={(value) => setData('skill_id', value)}
                label="Habilidades"
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <button
                type="submit"
                className="px-4 py-[6px] bg-black text-white dark:bg-white dark:text-black hover:bg-[#A9A9A9] 
                dark:hover:bg-gray-300 flex items-center mt-4 rounded-md"
              >
